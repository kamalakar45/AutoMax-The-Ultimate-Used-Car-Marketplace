...
def list_view(request):
    if request.method == 'POST':
        try:
            ...
            print(e)
            messages.error(request,'A error occured while posting the listing.')
...
def listing_view(request,id):
    try:
        ...
        messages.error(request,f'Invalid UID {id} was provided for lisitng.')
        return redirect('home')
...
def edit_view(request,id):
    try:
        ...
        messages.error(request,f'An error occured while trying to excess the edit page. ')
        return redirect('home')
...
def inquire_listing_using_email(request,id):
    ...
    try:
        ...
        send_mail(emailSubject,emailMessage,'kamalsai611@gmail.com',[listing.seller.user.email], fail_silently=True)
